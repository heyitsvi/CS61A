...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b[I*V*D*]+', text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'CS\s?[0-9]+(A|B|C)?', post, re.I))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'([0-1]?[0-9]{1}:[0-6][0-9][A-Z]*|[2]?[0-4]:[0-6][0-9])', text)
...
def area_codes(text):
    ...
    result = [x.group(1) for x in matches]
    return result
