...
def deep_map_mut(fn, link):
    ...
    if link is Link.empty:
        ...
        print("DEBUG:", link.first.first)
        link.first.first = fn(link.first.first)
        print("DEBUG: rest", link.first.rest)
        if link.first.rest:  
            return deep_map_mut(fn, link.first.rest)
        else:
...
def two_list(vals, amounts):
    ...
    def helper(index):
        if index == len(amounts):
            return Link.empty
        elif amounts[index] == 1:
            return Link(vals[index], helper(index + 1))
        else:
            amounts[index] -= 1
            return Link(vals[index], helper(index))
    return helper(0)
