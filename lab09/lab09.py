...
def merge(incr_a, incr_b):
    ...
    while True:
        ...
        if next_a is None and next_b is not None:
            yield next_b
            next_b = next(iter_b, None)
        if next_a is not None and next_b is None:
            yield next_a
            next_a = next(iter_a, None)
...
def insert(link, value, index):
    ...
    def helper(link, count):
        ...
        if index == count:
            original_value = link.first
            link.first = value
            link.rest = Link(original_value, link.rest)
        else:
            return helper(link.rest, count + 1)
    return helper(link, 0)
...
def get_all_paths(t):
    all_paths = []
    path_list = []
    path_list.append(t.label)
    def helper(t):
        if t.is_leaf():
            return 
        for b in t.branches:
            path_list.append(b.label)
            helper(b)
            print(path_list)
        # paths = [t.label] + [helper(b) for b in t.branches]
    return helper(t)
